...
/// Ctrl の 47 枠: 英字 22（C / X / V / Z は固定のキー・R-25 / R-83）＋ 数字 10 ＋ F キー 12 ＋ Enter・BackSpace・Delete。Ctrl+Shift は作らない。
...
public static class KeySlotList
{
    ...
    private static List<KeyBinding> Build()
    {
        ...
        for (var c = 'A'; c <= 'Z'; c++)
            if (c is not ('C' or 'X' or 'V' or 'Z')) slots.Add(new KeyBinding(Vk.Letter(c), Ctrl: true));
