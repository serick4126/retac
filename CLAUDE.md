...
`docs/handoff.md`（これまでの記録）と `docs/next_session_prompt.md`（次に始めるときの指示）は、**別々のもの**である。
次のどれかに当たったら、**両方を**更新して `docs/` でコミットする。片方だけにしない（次のプロンプトだけを書き換え続けて、記録が 2 Phase ぶん抜けたことがある）。

- Phase・段・計画の区切り（「区切り A」など）の作業と実機確認が終わったとき
- 利用者が作業を止める・セッションを終えると言ったとき
- リリースしたとき

`handoff.md` の末尾の「セッション記録」に、その区切りの節を足す（区切りの途中なら、その Phase の節を書き足す）。書くこと:

- 入れたもの（仕様 ID とコミットの対応）と、確定した仕様・利用者の決定
- 計画から変えた点と、実機確認の指摘で直した点（CHANGELOG に書く材料になる。書き漏らさない）
- 次の人がはまりそうな落とし穴・作業上の注意
- 残っている作業と、未コミット・未 push・未リリースのもの

`next_session_prompt.md` は、次に始めるときに読む指示だけにする（状況・やること・注意）。記録の中身は `handoff.md` を指せばよい。
...
- GPL など、ライセンスが合わないコードは断片も識別子も混ぜない。挙動を自分の言葉で整理し、公開資料だけを見て別に実装する。
  参考にしたソースを見ながら書かない。依存ライブラリや SDK を足すときは、先にライセンスを確認する。
